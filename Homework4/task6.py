...
def create_phone_number(num_tuple):
    b = ''.join(str(num_tuple))   # кортеж преобразовали в строку
    b = b.replace(', ', '')   # убираем запятые и пробелы
    str_phone = f'{b[0:4]}{b[-1]} {b[4:7]}-{b[-5:-1]}'   # порядок цифр, скобок и пробелов
    return str_phone
