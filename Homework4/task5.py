...
def josephus_task(num_people, kill_num):
    ...
    for i in range(1, num_people + 1):
        spisok_survivor.append(i)   # сформирован список для вычисления выживших
    while len(spisok_survivor) > 1:   # зацикливаем, пока длина списка >1
        for a in range(0, kill_num - 1):   # добавляем по 1 в конец все элементы до значения kill_num
            spisok_survivor.append(spisok_survivor[a])   # добавляем (вытаскиваем) выживших в конец списка
        del spisok_survivor[:kill_num]   # удаляем все элементы списка до значения kill_num
    survivor = spisok_survivor[0]   # оставляем 1 цифры
    return survivor
