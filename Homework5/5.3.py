...
def everything_for_your_cat(cats_data):
    test_dict = {}
    for k in cats_data:
        owner = f"{k[2]} {k[3]}:"   # в строку запишем имя и фамилия владельца
        pet = f'{k[0]}, {k[1]}'   # в строку запишем кличку и возраст котика
        if not test_dict.get(owner):   # если нет владельца в словаре
            test_dict[owner] = pet   # то значение ключа владельца записываем строку с котиком
        else:
            test_dict[owner] += f"; {pet}"   # то значением ключа владельца добавляем сумму строк из питомцев
    our_str = ''
    for owner in test_dict:
        our_str += f"{owner} {test_dict[owner]}\n"   # добавляем в одну строку и выводим каждого влад. с котиком с новой
    return our_str
