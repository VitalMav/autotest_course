...
def repeats(our_str):
    """
    :param our_str: строка на вход из букв
    :return: new_str: новая строка вида "буква_цифра", где строки и цифра - сколько раз буква встретилась в строке
    """
    tmp_dict = {}   # задаем временный словарь, куда запишем сколько раз встретили букву
    new_str = ''   # в процессе итерации по строке
    for bukva in our_str:
        if tmp_dict.get(bukva):   # если в словаре есть буква
            tmp_dict[bukva] += 1   # увеличиваем значение ключа буквы в словаре
            new_str += f"{bukva}_{tmp_dict[bukva]}"   # записываем в строку букву и значение ключа по букве из словаря
        else:
            tmp_dict[bukva] = 1   # если в словаре буквы нет, то значение ключа будет 1
            new_str += f"{bukva}_{1}"   # в строку пишем букву и значение 1
    return new_str
